...
mod ensemble_integration_test {
    ...
    async fn test_onnx_ensemble_integration() {
        ...
        if let Some(ref ensemble_config) = security_config.ensemble_config {
            println!(
                "   - Voting Strategy: {:?}",
                ensemble_config.voting_strategy
            );
        ...
        let deepset_model =
            std::path::Path::new("onnx_models/deepset_deberta-v3-base-injection.onnx");
        let protectai_model =
            std::path::Path::new("onnx_models/protectai_deberta-v3-base-prompt-injection-v2.onnx");
        ...
        if security_manager.is_enabled() {
            ...
            for (text, expected_type) in test_cases {
                ...
                match security_manager.scan_content(&content).await {
                    ...
                    Ok(None) => {
                        println!(
                            "   ⚠️  Scan returned None (scanner may not be properly initialized)"
                        );
                    }
                    Err(e) => {
