...
mod security_scanner_tests {
    ...
    async fn test_security_manager_different_policies() {
        ...
        for (policy, name) in policies {
            ...
            match &policy {
                ActionPolicy::Block => {
                    assert!(should_block, "Block policy should return true for should_block");
                    assert!(!should_ask, "Block policy should return false for should_ask_user");
                }
                ActionPolicy::AskUser => {
                    assert!(!should_block, "AskUser policy should return false for should_block");
                    assert!(should_ask, "AskUser policy should return true for should_ask_user");
                }
                ActionPolicy::LogOnly | ActionPolicy::Warn | ActionPolicy::Sanitize => {
                    assert!(!should_block, "{:?} policy should return false for should_block", policy);
                    assert!(!should_ask, "{:?} policy should return false for should_ask_user", policy);
                }
            }
    ...
    fn test_threat_level_thresholds() {
        ...
        for threshold in thresholds {
            ...
            for threat_level in &threat_levels {
                ...
                assert_eq!(should_block, expected_block, 
