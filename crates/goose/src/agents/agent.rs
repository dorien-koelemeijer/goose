...
use crate::tool_monitor::{ToolCall as MonitorToolCall, ToolMonitor};
use regex::Regex;
...
use crate::type_conversion::{rmcp_tool_to_mcp_core, mcp_core_tool_to_rmcp};
use mcp_core::{ToolError, ToolResult};
use mcp_core::tool::ToolAnnotations;
use rmcp::model::{Content, GetPromptResult, Prompt, ServerNotification, Role, Tool};
...
use crate::message::push_message;
...
impl Agent {
    ...
    pub async fn get_prefixed_tools(&self) -> ExtensionResult<Vec<mcp_core::Tool>> {
        ...
        for frontend_tool in frontend_tools.values() {
            tools.push(rmcp_tool_to_mcp_core(&frontend_tool.tool));
        }
    ...
    pub async fn dispatch_tool_call(
        ...
        if let Some(monitor) = self.tool_monitor.lock().await.as_mut() {
            let tool_call_info = MonitorToolCall::new(tool_call.name.clone(), tool_call.arguments.clone());
        ...
        let result: ToolCallResult = if sub_recipe_manager.is_sub_recipe_tool(&tool_call.name) {
            // TODO: Need to pass TasksManager - for now create a placeholder
            let tasks_manager = crate::agents::subagent_execution_tool::tasks_manager::TasksManager::new();
            sub_recipe_manager
                .dispatch_sub_recipe_tool_call(&tool_call.name, tool_call.arguments.clone(), &tasks_manager)
                .await
            ...
            ExtensionConfig::Frontend {
                ...
                for tool in tools {
                    let frontend_tool = FrontendTool {
                        ...
                        tool: mcp_core_tool_to_rmcp(tool),
                    };
    ...
    pub async fn list_tools(&self, extension_name: Option<String>) -> Vec<mcp_core::Tool> {
        ...
        if extension_name.is_none() || extension_name.as_deref() == Some("platform") {
            // Add platform tools - convert from rmcp to mcp_core
            prefixed_tools.extend([
                rmcp_tool_to_mcp_core(&platform_tools::search_available_extensions_tool()),
                rmcp_tool_to_mcp_core(&platform_tools::manage_extensions_tool()),
                rmcp_tool_to_mcp_core(&platform_tools::manage_schedule_tool()),
            ]);

            // Add resource tools if supported
            if extension_manager.supports_resources() {
                prefixed_tools.extend([
                    rmcp_tool_to_mcp_core(&platform_tools::read_resource_tool()),
                    rmcp_tool_to_mcp_core(&platform_tools::list_resources_tool()),
                ]);
            }
        }
        ...
        if extension_name.is_none() {
            let sub_recipe_manager = self.sub_recipe_manager.lock().await;
            // Convert sub recipe tools from rmcp to mcp_core
            prefixed_tools.extend(
                sub_recipe_manager.sub_recipe_tools.values()
                    .map(|tool| rmcp_tool_to_mcp_core(tool))
            );
        }
    ...
    pub async fn list_tools_for_router(
        ...
        match strategy {
            Some(RouterToolSelectionStrategy::Vector) => {
                prefixed_tools.push(rmcp_tool_to_mcp_core(&router_tools::vector_search_tool()));
            }
            Some(RouterToolSelectionStrategy::Llm) => {
                prefixed_tools.push(rmcp_tool_to_mcp_core(&router_tools::llm_search_tool()));
            }
            None => {}
        }
    ...
    async fn pre_scan_file_content(
        ...
        let content_for_scanning = vec![rmcp::model::Content::text(file_content)];
                ...
                for content in &latest_user_message.content {
                    match content {
                        MessageContent::Text(text_content) => {
                            text_contents.push(text_content.raw.text.clone());
                            all_content_for_scanning.push(Content::text(text_content.raw.text.clone()));
                        }
                        MessageContent::Image(image_content) => {
                            has_image_content = true;
                            tracing::info!(
                                mime_type = %image_content.raw.mime_type,
                                data_length = image_content.raw.data.len(),
                            ...
                            all_content_for_scanning.push(Content::text(format!(
                                "Image data ({}): {}", 
                                image_content.raw.mime_type,
                                if image_content.raw.data.len() > 200 {
                                    format!("{}...", &image_content.raw.data[..200])
                                } else {
                                    image_content.raw.data.clone()
                                }
                            )));
                        }
                        ...
                        _ => {
                            ...
                            if let Some(text) = content.as_text() {
                                text_contents.push(text.text.clone());
                                all_content_for_scanning.push(Content::text(text.text.clone()));
                            }
                        }
                    }
                }
                                            ...
                                            if let Some(security_manager) = &*self.security_manager.lock().await {
                                                ...
                                                if let Ok(Some(scan_result)) = security_manager.scan_content_with_type(content, crate::security::config::ContentType::ToolResult).await {
                                                    ...
                                                    let content_text = content.iter()
                                                        .filter_map(|c| c.as_text().map(|t| &t.text))
                                                        .collect::<Vec<_>>()
                                                        .join("\n");
