...
use crate::types::{ContentType, ModelConfig, ResponseMode, ScanResult};
use anyhow::Result;
#[cfg(feature = "onnx")]
...
impl OnnxModelScanner {
    ...
    async fn analyze_text_with_threshold(
        ...
        let scanner =
            crate::onnx::OnnxScanner::from_config(&self.model_config, self.response_mode.clone())?;
...
impl ModelScanner for OnnxModelScanner {
    async fn scan_text(&self, text: &str, content_type: ContentType) -> Result<ScanResult> {
        ...
        self.analyze_text_with_threshold(text, content_type, threshold)
            .await
    }
    ...
    fn get_threshold_for_content_type(&self, content_type: &ContentType) -> f32 {
        self.model_config
            .get_threshold_for_content_type(content_type)
    }
}
